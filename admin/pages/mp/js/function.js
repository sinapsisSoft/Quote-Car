...
function createTableEdit(json) {
    let thead = '<thead><tr><th>Número</th><th>Nombre del Mantenimiento</th><th>Selección</th><th>Acción</th></tr></thead>';
    let tbody = '<tbody>';
    let tfoot = ' <tfoot></tfoot> ';
    for (let i = 0; i < json.length; i++) {

        tbody += '<tr id="row_' + json[i]["id_mp"] + '"><td>' + json[i]["id_mp"] + '</td><td><input type="text" class="form-control" value="' + json[i]["name_mp"] + '" disabled></td><td><input type="radio" onclick="getRow(this.value)" name="items" value="' + json[i]["id_mp"] + '"></td><td><button type="button" onclick="changeCost(' + "'" + json[i]["id_mp"] + "'" + ')"class="btn btn-primary mb-2" disabled><i class="fas fa-save"></i></button></td></tr>';
    }
    tbody += '</tbody>';
